...
function handlenonMemberBtn() {
    ...
    imageList.forEach( x => {
      ...
      location.href=`http://127.0.0.1:5500/Main%20project/index.html?image=${imageList.indexOf(x_index)}`;
}
...
function handeMemberLogin(data) {
    ...
    imageList.forEach( x => {
     ...
     location.href= `http:\/\/127.0.0.1:5500/Main%20project/index.html?image=${imageList.indexOf(x_index)}&&uid=${data}`;
}
